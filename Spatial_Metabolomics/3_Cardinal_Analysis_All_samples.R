...
print("Adding Segmentation Annotations to Normalised and Binned Sample ...... ")
...
print("Converting from Cardinal Object to Seurat Object ......... ")

cardinal_to_seurat <- function(data,run_name){
...
saveRDS(data, paste0(folder,"All_Samples_Seurat.RDS"))
###########################


### Find Differentially Expressed Peaks between treated and sample Tumour samples
print("Finding Differentially Expressed Peaks ............... ")

Idents(data) <- "ssc_region"
tumour_regions <- subset(data, idents = 3)


Idents(tumour_regions) <- "treatment_class"
markers <- FindAllMarkers(tumour_regions, assay = "Spatial", slot = "data", min.pct = 0.25, logfc.threshold = 0.25, only.pos = TRUE)
markers <- markers %>% dplyr::filter(p_val_adj <= 0.05)

write.csv(markers, paste0(folder, "All_Samples_Tumour_Peaks.csv"))
##############################################################################################################################
...
### OLD CARDINAL ANALYSIS

#### Plotting Segmentation Results and Drug Peaks ############################################################################
