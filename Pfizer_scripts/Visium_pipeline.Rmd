...
while (idx <= length(vis_sample_list)){
  plot <- plot_visium_clusters(vis_clusters[[idx]], vis_sample_list[idx], output_dir, adding_graph = TRUE)
  plot_list[[idx]] <- plot 
  idx <- idx + 1
}
...
cell_spots <- function(data, sample, outdir, adding_graph = FALSE){
...
while (idx <= length(vis_sample_list)){
  ...
  plot <- cell_spots(vis_lb_tf[[idx]], vis_sample_list[idx], output_dir, adding_graph = TRUE)
  plot_list[[idx]] <- plot
  idx <- idx + 1
}
...
```{r}
tumour_only <- list()
for (sample in names(vis_lb_tf)){
    Idents(vis_lb_tf[[sample]]) <- "celltype_major"
    sub <- subset(vis_lb_tf[[sample]], ident = "Cancer Epithelial")
    tumour_only[[sample]] <- sub
}

p.features <- SelectIntegrationFeatures(tumour_only, nfeatures = 3000, verbose = FALSE)


p.prepsct <- PrepSCTIntegration(object.list = tumour_only, 
