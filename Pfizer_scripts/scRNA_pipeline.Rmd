...
sc_pipeline <- function(ID, inputdir, outputdir) {
  ...
  # run sctransform
  sc_data <- SCTransform(sc_data, vars.to.regress = "percent.mt", verbose = FALSE)
...
ggsave(paste0(outdir_ref,"Reference_umap_plots.pdf"), plot1/plot2, height = 15) 
...
while (idx <= length(input_sample_list)){
  ...
  meta_idx <- which(metadata$Subject.ID == new_sample_id)
...
plot4 <- DimPlot(scRNA.combined, group.by = "celltype_major")
