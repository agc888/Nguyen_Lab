...
marker_genes <- split(df$gene, df$cluster)
...
for(i in seq(1,length(lbtr_list))){
  plot <- DotPlot(lbtr_list[[i]], features = top_5 ,group.by = "seurat_clusters") + theme(axis.text.x = element_text(angle = 90, vjust = 0.5, hjust=1), axis.text.x.top = element_text(angle = 90, vjust = 0.5, hjust=1))+NoLegend()
  heatmaps[[input_sample_list[[i]]]] <- plot
