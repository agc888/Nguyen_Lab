...
####Weird name -> rename #### 
important_metabolomics$`ï..Compound.name`[17] <- "ketoglutarate"
#############################

### Remove_duplicates ###
rename_list <- c()
for (met in important_metabolomics$`ï..Compound.name`){
...
important_metabolomics$`ï..Compound.name` <- rename_list
...
plot_metabolites <- function(data, data_list, run_name){
  ...
  for (index in rownames(data_list)){
    ...
    if(as.integer(mz) > 161){
      metabolite_name <- data_list$`ï..Compound.name`[index]
...
pixelData(run1_pre)@listData$tissue <- keep_clusters
